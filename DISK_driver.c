...
int openfile(char *name){
	...
	for(int i = 0; i < 20; i++){
		if(fat[i].current_location == -1){
			...
			//write the data in new file to data blocks in partition

			int offset = sizeof(partition)+sizeof(fat)+block_index*partition.block_size;
			fseek(part, offset, SEEK_SET);//go to the global pointer block_index which is the first empty block in the data section of partition
			int numblock = 0;
			
			while(1){
				//write one block at a time
				char *data;
				fread(data, partition.block_size, 1, newfile);
				fwrite(data, partition.block_size, 1, part);
				
				temp->blockPtrs[numblock] = block_index; //record the block number for each block written
				numblock++; //keep track of number of blocks with variable numblock
				block_index++; //increment global pointer
				if(feof(newfile)) {
					break ;
				}
			...
			temp->file_length = numblock;
			temp->current_location = 0;
			
			fat[i] = *temp;
			return openfile(name);//now that the new file has been added to fat, call the function again to open the file
		}
		if(i == 19) return -1;
	}
	fclose(part);
